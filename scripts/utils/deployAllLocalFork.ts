...
import { deployFakeLp } from "./deployFakeLpTokens";
...
const signerAddr = "0x10fc45741bfE5D527c1b83Fe0BD70fC96D7ec30F";
...
async function main() {
  try {
    ...
    const mockLpAddress = await deployFakeLp("USDT-USDT+", "1000000");
    if (!mockLpAddress) {
      throw new Error("Failed to deploy fake lp");
    }
    ...
    let tx = await (
      overlayerReferralContract.connect(admin) as Contract
    ).setMinter(singleStableStakeAddr, defaultTransactionOptions);
