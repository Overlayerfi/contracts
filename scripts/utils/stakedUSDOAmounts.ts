...
async function main() {
  ...
  const soverlayerWrap = new ethers.Contract(addr, abi.abi, signer);
  const totalSupply = ethers.formatEther(
    await soverlayerWrap.connect(signer).totalSupply()
  );
  const totalAssets = ethers.formatEther(
    await soverlayerWrap.connect(signer).totalAssets()
  );
