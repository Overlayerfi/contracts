...
async function main() {
  ...
  for (let i=0; i<signerNum; i++) {
    signers.push(new ethers.Wallet(ethers.Wallet.createRandom().privateKey, provider));
  ...
  const ovaReferralContract = new ethers.Contract(ovaReferralAddress, OVAREFERRAL_ABI.abi);
  const curveStableStakingContract = new ethers.Contract(curveStableStakingPoolAddress, CURVESTABLESTAKINGPOOL_ABI.abi)
  const curveStableStakingLpContract= new ethers.Contract(stakedCurveLpAddress, ERC20_ABI.abi)

  try {
    // Add codes
    for (let i=0; i<codesNum; i++) {
      let tx = await ovaReferralContract.connect(signers[i]).addCodeSelf(codes[i]);
      console.log("Transaction sent! Waiting for confirmation...");
    ...
    // Consume codes
    for (let i=0; i<codesNum; i++) {
      let tx = await ovaReferralContract.connect(signers[i+codesNum]).consumeReferral(codes[i], signers[i+codesNum]);
      console.log("Transaction sent! Waiting for confirmation...");
      console.log(`Transaction Hash: ${tx.hash}`);
      tx = await tx.wait();
      console.log("Transaction confirmed!");
  ...
  try {
    ...
    for (let i=0; i<codesNum; i++) {
      let tx = await curveStableStakingLpContract.connect(owner).transfer(signers[i+codesNum], ethers.parseEther('1'));
      console.log("Transaction sent! Waiting for confirmation...");
      console.log(`Transaction Hash: ${tx.hash}`);
      tx = await tx.wait();
      console.log("Transaction confirmed!");
      ...
      tx = await curveStableStakingLpContract.connect(signers[i+codesNum]).approve(curveStableStakingPoolAddress, ethers.MaxUint256);
      tx = await tx.wait();
    }
  } catch(e) {
    console.error(e);
  ...
  try {
    // Stake
    for (let i=0; i<codesNum; i++) {
      let tx = await curveStableStakingContract.connect(signers[i+codesNum]).deposit(pid, ethers.parseEther('1'));
      console.log("Transaction sent! Waiting for confirmation...");
      console.log(`Transaction Hash: ${tx.hash}`);
      tx = await tx.wait();
      console.log("Transaction confirmed!");
    ...
    // Harvest
    for (let i=0; i<codesNum; i++) {
      let tx = await curveStableStakingContract.connect(signers[i+codesNum]).harvest(pid,);
      console.log("Transaction sent! Waiting for confirmation...");
      console.log(`Transaction Hash: ${tx.hash}`);
      tx = await tx.wait();
      console.log("Transaction confirmed!");
    ...
    // Harvest
    for (let i=0; i<codesNum; i++) {
      let tx = await curveStableStakingContract.connect(signers[i+codesNum]).withdraw(pid, ethers.parseEther('1'));
      console.log("Transaction sent! Waiting for confirmation...");
      console.log(`Transaction Hash: ${tx.hash}`);
      tx = await tx.wait();
      console.log("Transaction confirmed!");
