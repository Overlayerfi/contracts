...
describe ('USDx', function () {
  ...
  describe('Set Minter', function () {
    it ("Should set the minter", async function () {
      ...
      const adminAddress = await admin.getAddress();
      const aliceAddress = await alice.getAddress();
      await expect(await contract.connect(admin).setMinter(aliceAddress)).to.emit(contract, 'MinterUpdated');
  ...
  describe('Mint', function () {
    it ("Should mint to", async function () {
      ...
      await expect(await contract.connect(alice).mint(bobAddress, ethers.parseEther('1'))).to.emit(contract, 'Transfer');
      expect(await contract.balanceOf(bobAddress)).to.equal(ethers.parseEther('1'));
      expect(await contract.balanceOf(adminAddress)).to.equal(ethers.parseEther('0'));
      expect(await contract.balanceOf(aliceAddress)).to.equal(ethers.parseEther('0'));
    });
  });
  ...
  describe('Burn', function () {
    it ("Should burn from", async function () {
      ...
      await contract.connect(admin).burnFrom(bobAddress, ethers.parseEther('1'));
      expect(await contract.balanceOf(bobAddress)).to.equal(ethers.parseEther('0'));
    });
  });
});
