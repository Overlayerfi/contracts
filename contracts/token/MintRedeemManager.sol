...
import "./interfaces/IMintRedeemManager.sol";

/**
 * @title MintRedeemManager
 * @notice This contract mints and redeems the parent USDxM that inherits this contract
 */
contract MintRedeemManager is IMintRedeemManager, SingleAdminAccessControl, ReentrancyGuard {
  ...
  constructor(
    ...
    if (_parentDecimals == 0) revert InvalidDecimals();

    usdc = _usdc;
    usdt = _usdt;
    _assetsDestinationWallet = _newAssetDestinationWallet;
    _decimals = _parentDecimals;
  ...
  function transferToCustody(address asset, uint256 amount)
    external
    nonReentrant
  ...
  function validateInvariant(Order calldata order) internal view {
    if (order.collateral_usdc_amount / usdc.decimals !=
        order.collateral_usdt_amount / usdt.decimals) revert InvalidAssetAmounts(); 
    // Their sum must be equal to USDx amount
    if (order.collateral_usdc_amount / usdc.decimals +
        order.collateral_usdt_amount / usdt.decimals != order.usdx_amount / _decimals) revert InvalidAssetAmounts();
  }

  /// @notice Mint stablecoins from assets
  /// @param order A struct containing the mint order
  function mintInternal(Order calldata order)
    internal
    nonReentrant
    belowMaxMintPerBlock(order.usdx_amount)
  {
    ...
    _transferCollateral(order.collateral_usdc_amount, order.collateral_usdc, order.benefactor);
    _transferCollateral(order.collateral_usdt_amount, order.collateral_usdt, order.benefactor);
  }

  /// @notice Redeem stablecoins for assets
  /// @param order struct containing order details and confirmation from server
  function redeemInternal(Order calldata order)
    internal
    nonReentrant
    belowMaxRedeemPerBlock(order.usdx_amount)
  {
    ...
    _transferToBeneficiary(order.beneficiary, order.collateral_usdc, order.collateral_usdc_amount);
    _transferToBeneficiary(order.beneficiary, order.collateral_usdt, order.collateral_usdt_amount);
  }
  ...
  function _transferCollateral(
    ...
    if (!(asset == usdc.addr || asset == usdt.addr)) revert UnsupportedAsset();
